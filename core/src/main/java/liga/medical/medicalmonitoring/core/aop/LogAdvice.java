...
public class LogAdvice {
    ...
    public Object applicationLogger(ProceedingJoinPoint pjp) throws JsonProcessingException {
        ...
        logService.logDebug(new DebugDto(SystemType.MEDICAL_MONITORING, methodParams));

        try {
            object = pjp.proceed();
        } catch (Throwable e) {
            logService.logException(new ExceptionDto(SystemType.MEDICAL_MONITORING, methodParams));
            return object;
        }
        logService.logDebug(new DebugDto(SystemType.MEDICAL_MONITORING, methodParams));
